...
static String leadingZero(unsigned v)
{
    String out = "";
    v &= 0xff;
    if (v < 16)
...
void onEvent(ev_t event)
{
    ...
    case EV_JOINED:
    {
        ...
        log("netid: " + String(netid, DEC));
        log("devaddr: " + String(devaddr, HEX));
        String t = "AppSKey: ";
        for (size_t i = 0; i < sizeof(artKey); ++i)
        {
            if (i != 0)
                t += "-";
            t += leadingZero(artKey[i]);
        }
        log(t);
        t = "NwkSKey: ";
    ...
    case EV_TXCOMPLETE:
        log("EV_TXCOMPLETE (inc. RX win. wait)");
...
void ttn_join()
{
    ...
    if (!keysgood)
    {
        ...
        // Make LMiC initialize the default channels, choose a channel, and
        // schedule the OTAA join
        log("No session saved, joining from scratch");
        LMIC_startJoining();
    }
    else
    {
        log("Rejoining saved session");
